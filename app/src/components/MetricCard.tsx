...
export function MetricCard({ label, value, hint, loading = false }: { label: ReactNode; value: ReactNode; hint?: string; loading?: boolean }) {
  return (
    <div className="rounded-2xl border border-fog bg-paper-white p-8">
      <p className="flex items-center gap-1.5 text-caption text-graphite">{label}</p>
      {loading ? (
