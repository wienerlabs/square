...
describe("migrations", () => {
  it("applies all nine in order, reverts the last one, and re-applies it", async () => {
    ...
    try {
      ...
      expect((await migrate(db, MIGRATIONS_DIR, "down")).applied).toEqual(["0009_x402_last_checked"]);
      expect(await migrationStatus(db, MIGRATIONS_DIR)).toEqual({ applied: ALL.slice(0, 8), pending: ["0009_x402_last_checked"] });
      expect(await columnNames(db, "x402_payments")).not.toContain("last_checked_at");

      expect((await migrate(db, MIGRATIONS_DIR, "up")).applied).toEqual(["0009_x402_last_checked"]);
  ...
  it("up, down nine steps, up leaves the schema identical", async () => {
    const db = await pgliteDatabase();
    try {
      await migrate(db, MIGRATIONS_DIR, "up");
      const first = await schemaSnapshot(db);

      expect((await migrate(db, MIGRATIONS_DIR, "down", 9)).applied).toEqual([...ALL].reverse());
